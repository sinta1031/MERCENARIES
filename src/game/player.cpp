...
#define OILGRAVITY			( -0.05f )	//オイルに入った時の重力
#define JUMPPOWER           ( 3.0f )	//ジャンプパワー
#define OILJUMPPOWER		( 1.5f )	//足を取られている時のジャンプパワー
...
void  CPlayer::Draw()
{
	...
	for (int i = 0; i < m_KeyhaveCount; i++)
	{
		//第1引数：X座標、2：Y座標、3：拡大縮小率、4：回転速度(ラジアン角)
		//5：画像ハンドル、6：透過処理を有効にするか
		DrawRotaGraph(m_PosX + (25 * i), m_PosY, 0.2, 0.0, m_KeyHndl, TRUE);
	}
...
void CPlayer::jump()
{
	if (m_gravity == 0.0f)
	{
		if (m_eState != PLAYER_STATE_OIL)
		{
			m_gravity = JUMPPOWER;
		}
		else if (m_eState == PLAYER_STATE_OIL)
		{
			m_gravity = OILJUMPPOWER;
		}
	}	
}
...
//鍵に触れた時の処理
void CPlayer::KeyHitCalc()
{
	m_KeyhaveCount += 1;
}

//ゴールの扉に触れた時の処理
void CPlayer::GoalHitCalc()
{
	if (m_KeyhaveCount >= 1)
	{
		m_GoalActive = true;
